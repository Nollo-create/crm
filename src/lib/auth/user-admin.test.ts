...
describe("roleChangeError", () => {
  it("allows an admin to change a member's role", () => {
    expect(roleChangeError(admin, memberTarget, "admin", 1)).toBeNull();
  });
  ...
  it("only an owner may grant or revoke owner", () => {
    expect(roleChangeError(admin, memberTarget, "owner", 1)).toBe("Only an owner can manage owners.");
    expect(roleChangeError(admin, ownerTarget, "member", 2)).toBe("Only an owner can manage owners.");
