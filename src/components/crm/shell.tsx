    ...
    <div className="flex min-h-screen bg-background">
            ...
            <div key={group.title ?? gi} className="space-y-0.5">
              ...
              {group.items.map((item) => {
                ...
                return item.soon ? (
                  ...
                  <Link key={item.href} href={item.href} className={cls} title={collapsed ? item.label : undefined}>
                    {inner}
                  </Link>
                );
              })}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b border-border bg-background/80 px-4 backdrop-blur">
          <button
            className="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-muted-foreground hover:bg-secondary hover:text-foreground md:hidden"
            title="Menu"
          >
            <PanelLeft size={16} />
          </button>
