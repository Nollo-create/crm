...
An AI B2B Sales OS for the Sajtpress platform — a **separate** Next.js 15 app at
`crm.sajtpress.rs`, built to connect to the webapp (`webapp.sajtpress.rs`) but to
...
1. **cPanel → Setup Node.js App** → create an app: Application root
   `repositories/crm`, Application URL `crm.sajtpress.rs`, startup file
