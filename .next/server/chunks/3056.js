exports.id=3056,exports.ids=[3056],exports.modules={28303:a=>{function b(a){var b=Error("Cannot find module '"+a+"'");throw b.code="MODULE_NOT_FOUND",b}b.keys=()=>[],b.resolve=b,b.id=28303,a.exports=b},31100:(a,b,c)=>{"use strict";c.d(b,{BB:()=>e,Oj:()=>n,Qd:()=>h,aD:()=>i,xM:()=>f,zk:()=>k});let d=["new","working","qualified","unqualified","converted"],e={new:"New",working:"Working",qualified:"Qualified",unqualified:"Unqualified",converted:"Converted"};function f(a){return d.includes(a)}let g=["web","referral","event","cold","other"],h={web:"Website",referral:"Referral",event:"Event",cold:"Cold outreach",other:"Other"};function i(a){return g.includes(a)}let j=["low","normal","high"];function k(a){return j.includes(a)}let l={name:"l.name",company:"l.company",source:"l.source",status:"l.status",score:"l.lead_score",created:"l.id"},m=new Set(Object.keys(l));function n(a,b){let c=m.has(a)?l[a]:l.score;return`ORDER BY ${c} ${1===b?"ASC":"DESC"}, l.id DESC`}},47554:(a,b,c)=>{"use strict";c.d(b,{RO:()=>X,IP:()=>F,J5:()=>am,hi:()=>aq,wW:()=>ai,I9:()=>ap,W3:()=>ao,dt:()=>aj,_5:()=>al,Fe:()=>ak,xS:()=>an,PH:()=>D,kF:()=>E,bC:()=>T,vG:()=>S,KP:()=>af,cQ:()=>aD,_:()=>aA,Iq:()=>aI,eK:()=>v,ro:()=>M,tR:()=>_,jw:()=>aQ,UT:()=>ag,Ml:()=>C,Bw:()=>r,Lh:()=>aK,RC:()=>A,MO:()=>K,iG:()=>V,sr:()=>ae,Z4:()=>ay,Ll:()=>aL,C1:()=>x,bl:()=>G,NE:()=>P,DT:()=>aa,SA:()=>aB,Fg:()=>aR,aH:()=>aG,ht:()=>aN,kl:()=>aO,s3:()=>Y,n:()=>Z,d5:()=>aH,mm:()=>w,td:()=>B,I8:()=>I,ik:()=>J,jQ:()=>L,Ey:()=>Q,Et:()=>N,e2:()=>R,fI:()=>W,_J:()=>av,BH:()=>ac,tB:()=>aw,zJ:()=>au,W5:()=>at,fS:()=>as,LB:()=>ar,c5:()=>ax,bW:()=>aJ,hE:()=>U,rM:()=>ad,lY:()=>aP,tl:()=>aM,zK:()=>aF,JT:()=>y,lH:()=>z,CI:()=>H,Kd:()=>O,Q6:()=>ab,uI:()=>aE,L_:()=>aC,fs:()=>aS});var d=c(29382),e=c(50505);let f={name:"c.name",industry:"c.industry",contacts:"contacts",openValue:"open_value",annualValue:"c.annual_value",score:"c.lead_score",health:"health_rank",lastActivity:"last_activity"},g=new Set(Object.keys(f));function h(a,b,c){let d=Math.min(100,Math.max(1,Math.floor(b)||25)),e=Math.max(1,Math.ceil(c/d)),f=Math.min(Math.max(1,Math.floor(a)||1),e);return{page:f,pageSize:d,offset:(f-1)*d,pageCount:e}}let i={name:"ct.name",role:"ct.role",company:"co.name",email:"ct.email",influence:"ct.influence"},j=new Set(Object.keys(i));var k=c(31100);let l={title:"d.title",company:"co.name",value:"d.value",stage:`FIELD(d.stage, ${e.w7.map(a=>`'${a}'`).join(", ")})`,expectedClose:"d.expected_close",created:"d.id"},m=new Set(Object.keys(l)),n={created:"a.created_at",type:"a.type",company:"co.name"},o=new Set(Object.keys(n));c(85750);let p=globalThis;function q(){return p.__cmsPool||(p.__cmsPool=d.createPool({host:process.env.DB_HOST||"localhost",port:Number(process.env.DB_PORT||3306),user:process.env.DB_USER||"",password:process.env.DB_PASSWORD||"",database:process.env.DB_NAME||"",waitForConnections:!0,connectionLimit:5,charset:"utf8mb4_general_ci"})),p.__cmsPool}async function r(){if(!process.env.DB_NAME)return!1;try{return await q().query("SELECT 1"),!0}catch{return!1}}async function s(a,b,c,d){let[e]=await a.query("SELECT COUNT(*) AS n FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND COLUMN_NAME = ?",[b,c]);0===Number(e[0]?.n??0)&&await a.query(`ALTER TABLE \`${b}\` ADD COLUMN ${d}`)}function t(){return p.__crmSchema||(p.__crmSchema=(async()=>{let a=q();await a.query(`
     ...
     FROM crm_companies WHERE organization_id = ?`,[a]),[d]=await b.query("SELECT COALESCE(SUM(value), 0) AS won FROM crm_deals WHERE organization_id = ? AND stage = 'won'",[a]);return{customers:Number(c[0]?.customers??0),atRisk:Number(c[0]?.at_risk??0),arr:Number(c[0]?.arr??0),won:Number(d[0]?.won??0)}}async function D(a,b){let c=b.filter(a=>Number.isInteger(a)).slice(0,500);if(!c.length)return;await t();let d=c.map(()=>"?").join(","),e=q();await e.query(`DELETE FROM crm_activities WHERE organization_id = ? AND company_id IN (${d})`,[a,...c]),await e.query(`DELETE FROM crm_deals WHERE organization_id = ? AND company_id IN (${d})`,[a,...c]),await e.query(`DELETE FROM crm_contacts WHERE organization_id = ? AND company_id IN (${d})`,[a,...c]),await e.query(`DELETE FROM crm_companies WHERE organization_id = ? AND id IN (${d})`,[a,...c])}async function E(a,b,c){let d=b.filter(a=>Number.isInteger(a)).slice(0,500);if(!d.length)return;await t();let e=d.map(()=>"?").join(",");await q().query(`UPDATE crm_companies SET status = ? WHERE organization_id = ? AND id IN (${e})`,[c.slice(0,20),a,...d])}async function F(a,b,c){await t();let[d]=await q().query(`INSERT INTO crm_contacts (organization_id, company_id, name, role, email, phone, department, influence, mobile, linkedin, notes)
      ...
      WHERE d.id = ? AND d.organization_id = ? LIMIT 1`,[b,a]);return c[0]??null}async function Q(a,b,c=50){await t();let[d]=await q().query("SELECT * FROM crm_activities WHERE deal_id = ? AND organization_id = ? ORDER BY id DESC LIMIT ?",[b,a,c]);return d}async function R(a,b){await t();let[c]=await q().query("SELECT * FROM crm_deals WHERE contact_id = ? AND organization_id = ? ORDER BY updated_at DESC LIMIT 50",[b,a]);return c}async function S(a,b){await t();let c=await q().getConnection();try{await c.beginTransaction();let[d]=await c.query("SELECT company_id FROM crm_deals WHERE id = ? AND organization_id = ? FOR UPDATE",[b,a]),e=d[0];if(!e)return await c.rollback(),null;return await c.query("UPDATE crm_deals SET stage = 'won', probability = 100, closed_at = CURRENT_TIMESTAMP, loss_reason = '' WHERE id = ? AND organization_id = ?",[b,a]),await c.query("UPDATE crm_companies SET status = 'customer' WHERE id = ? AND organization_id = ? AND status <> 'customer'",[e.company_id,a]),await c.commit(),{companyId:e.company_id}}catch(a){throw await c.rollback(),a}finally{c.release()}}async function T(a,b,c){await t();let[d]=await q().query("SELECT company_id FROM crm_deals WHERE id = ? AND organization_id = ? LIMIT 1",[b,a]),e=d[0];return e?(await q().query("UPDATE crm_deals SET stage = 'lost', probability = 0, closed_at = CURRENT_TIMESTAMP, loss_reason = ? WHERE id = ? AND organization_id = ?",[c.slice(0,40),b,a]),{companyId:e.company_id}):null}async function U(a,b,c){await t();let[d]=await q().query("SELECT company_id FROM crm_deals WHERE id = ? AND organization_id = ? LIMIT 1",[b,a]),e=d[0];return e?(await q().query("UPDATE crm_deals SET stage = ?, closed_at = NULL, loss_reason = '' WHERE id = ? AND organization_id = ?",[c.slice(0,20),b,a]),{companyId:e.company_id}):null}async function V(a,b){await t(),await q().query("DELETE FROM crm_deals WHERE id = ? AND organization_id = ?",[b,a])}async function W(a,b){await t();let c=["d.organization_id = ?"],d=[a];if(b.q){c.push("(d.title LIKE ? OR co.name LIKE ?)");let a=`%${b.q}%`;d.push(a,a)}b.stage&&(c.push("d.stage = ?"),d.push(b.stage));let e="FROM crm_deals d JOIN crm_companies co ON co.id = d.company_id AND co.organization_id = d.organization_id",f=`WHERE ${c.join(" AND ")}`,g=q(),[i]=await g.query(`SELECT COUNT(*) AS n ${e} ${f}`,d),j=Number(i[0]?.n??0),{offset:k,pageSize:n,page:o,pageCount:p}=h(b.page,b.pageSize,j),r=function(a,b){let c=m.has(a)?l[a]:l.value;return`ORDER BY ${c} ${1===b?"ASC":"DESC"}, d.id DESC`}(b.sortKey,b.sortDir),[s]=await g.query(`SELECT d.*, co.name AS company_name ${e} ${f} ${r} LIMIT ? OFFSET ?`,[...d,n,k]);return{rows:s,total:j,page:o,pageCount:p}}async function X(a,b){await t();let[c]=await q().query("INSERT INTO crm_activities (organization_id, company_id, contact_id, deal_id, type, summary) VALUES (?, ?, ?, ?, ?, ?)",[a,b.companyId,b.contactId??null,b.dealId??null,(b.type??"note").slice(0,20),b.summary.slice(0,500)]);return c.insertId}async function Y(a,b,c=50){await t();let[d]=await q().query("SELECT * FROM crm_activities WHERE company_id = ? AND organization_id = ? ORDER BY id DESC LIMIT ?",[b,a,c]);return d}async function Z(a,b){await t();let c=["a.organization_id = ?"],d=[a];if(b.q){c.push("(a.summary LIKE ? OR co.name LIKE ?)");let a=`%${b.q}%`;d.push(a,a)}b.type&&(c.push("a.type = ?"),d.push(b.type));let e="FROM crm_activities a JOIN crm_companies co ON co.id = a.company_id AND co.organization_id = a.organization_id",f=`WHERE ${c.join(" AND ")}`,g=q(),[i]=await g.query(`SELECT COUNT(*) AS n ${e} ${f}`,d),j=Number(i[0]?.n??0),{offset:k,pageSize:l,page:m,pageCount:p}=h(b.page,b.pageSize,j),r=function(a,b){let c=o.has(a)?n[a]:n.created;return`ORDER BY ${c} ${1===b?"ASC":"DESC"}, a.id DESC`}(b.sortKey,b.sortDir),[s]=await g.query(`SELECT a.*, co.name AS company_name ${e} ${f} ${r} LIMIT ? OFFSET ?`,[...d,l,k]);return{rows:s,total:j,page:m,pageCount:p}}function $(a){return(0,e.DY)({hasWebsite:!!(a.website??"").trim(),employees:a.employees??null,industryMatch:!!a.industryMatch,annualValue:a.annualValue??0})}async function _(a,b){await t();let[c]=await q().query(`INSERT INTO crm_leads (organization_id, name, company, title, email, phone, source, status, industry, website, employees, annual_value, industry_match, lead_score, notes, priority, owner)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,[a,b.name.slice(0,190),(b.company??"").slice(0,190),(b.title??"").slice(0,120),(b.email??"").slice(0,190),(b.phone??"").slice(0,60),(b.source??"other").slice(0,30),(b.status??"new").slice(0,20),(b.industry??"").slice(0,120),(b.website??"").slice(0,300),b.employees??null,b.annualValue??0,+!!b.industryMatch,$(b),(b.notes??"").slice(0,500),(b.priority??"normal").slice(0,12),(b.owner??"").slice(0,120)]);return c.insertId}async function aa(a,b){await t();let[c]=await q().query("SELECT * FROM crm_leads WHERE id = ? AND organization_id = ? LIMIT 1",[b,a]);return c[0]??null}async function ab(a,b,c){await t(),await q().query(`UPDATE crm_leads SET name=?, company=?, title=?, email=?, phone=?, source=?, industry=?, website=?, employees=?, annual_value=?, industry_match=?, lead_score=?, notes=?, priority=?, owner=?
       WHERE id=? AND organization_id=?`,[c.name.slice(0,190),(c.company??"").slice(0,190),(c.title??"").slice(0,120),(c.email??"").slice(0,190),(c.phone??"").slice(0,60),(c.source??"other").slice(0,30),(c.industry??"").slice(0,120),(c.website??"").slice(0,300),c.employees??null,c.annualValue??0,+!!c.industryMatch,$(c),(c.notes??"").slice(0,500),(c.priority??"normal").slice(0,12),(c.owner??"").slice(0,120),b,a])}async function ac(a,b){await t();let c=["l.organization_id = ?"],d=[a];if(b.q){c.push("(l.name LIKE ? OR l.company LIKE ? OR l.email LIKE ?)");let a=`%${b.q}%`;d.push(a,a,a)}b.status&&(c.push("l.status = ?"),d.push(b.status)),b.source&&(c.push("l.source = ?"),d.push(b.source));let e=`WHERE ${c.join(" AND ")}`,f=q(),[g]=await f.query(`SELECT COUNT(*) AS n FROM crm_leads l ${e}`,d),i=Number(g[0]?.n??0),{offset:j,pageSize:l,page:m,pageCount:n}=h(b.page,b.pageSize,i),o=(0,k.Oj)(b.sortKey,b.sortDir),[p]=await f.query(`SELECT l.* FROM crm_leads l ${e} ${o} LIMIT ? OFFSET ?`,[...d,l,j]);return{rows:p,total:i,page:m,pageCount:n}}async function ad(a,b,c){await t(),await q().query("UPDATE crm_leads SET status = ? WHERE id = ? AND organization_id = ?",[c.slice(0,20),b,a])}async function ae(a,b){await t(),await q().query("DELETE FROM crm_leads WHERE id = ? AND organization_id = ?",[b,a])}async function af(a,b,c={}){await t();let d=await q().getConnection();try{await d.beginTransaction();let[e]=await d.query("SELECT * FROM crm_leads WHERE id = ? AND organization_id = ? FOR UPDATE",[b,a]),f=e[0];if(!f)return await d.rollback(),null;if(f.converted_company_id)return await d.commit(),{companyId:f.converted_company_id,dealId:null,createdCompany:!1};let g=0;if(c.companyId){let[b]=await d.query("SELECT id FROM crm_companies WHERE id = ? AND organization_id = ? LIMIT 1",[c.companyId,a]);b[0]&&(g=Number(b[0].id))}let h=0===g;if(h){let b=$({website:f.website,employees:f.employees,industryMatch:!!f.industry_match,annualValue:f.annual_value}),[c]=await d.query(`INSERT INTO crm_companies (organization_id, name, industry, city, website, employees, annual_value, status, account_manager, industry_match, lead_score)
       ...
       COUNT(*) AS n
     FROM crm_deals WHERE organization_id = ? GROUP BY owner ORDER BY won DESC, open DESC LIMIT 20`,[a]);return b.map(a=>({owner:String(a.owner),won:Number(a.won),open:Number(a.open),n:Number(a.n)}))}async function ap(a){await t();let[b]=await q().query(`SELECT DATE_FORMAT(expected_close, '%Y-%m') AS month, stage, COALESCE(SUM(value),0) AS value
     FROM crm_deals
     WHERE organization_id = ? AND stage NOT IN ('won','lost') AND expected_close IS NOT NULL
     GROUP BY month, stage ORDER BY month ASC`,[a]);return b.map(a=>({month:String(a.month),stage:String(a.stage),value:Number(a.value)}))}async function aq(a){await t();let[b]=await q().query("SELECT COUNT(*) AS n FROM crm_activities WHERE organization_id = ? AND created_at >= NOW() - INTERVAL 30 DAY",[a]);return Number(b[0]?.n??0)}async function ar(a,b,c){await t();let d=Number(b)||30,[e]=await q().query(`SELECT c.id, c.name,
     ...
     ORDER BY d.expected_close ASC LIMIT ?`,[a,Number(b)||5]);return c.map(a=>({id:Number(a.id),companyId:Number(a.company_id),companyName:String(a.company_name),title:String(a.title),days:Number(a.days)}))}async function at(a,b,c){await t();let d=Number(b)||60,[e]=await q().query(`SELECT id, name, company, lead_score FROM crm_leads
     WHERE organization_id = ? AND status IN ('new','working') AND lead_score >= ${d}
     ORDER BY lead_score DESC LIMIT ?`,[a,Number(c)||5]);return e.map(a=>({id:Number(a.id),name:String(a.name),company:String(a.company),score:Number(a.lead_score)}))}async function au(a,b,c){await t();let d=Number(b)||7,[e]=await q().query(`SELECT q.id, co.name AS company_name, DATEDIFF(NOW(), q.created_at) AS days
     ...
     ORDER BY q.created_at ASC LIMIT ?`,[a,Number(c)||5]);return e.map(a=>({id:Number(a.id),companyName:String(a.company_name),days:Number(a.days)}))}async function av(a){await t();let[b]=await q().query("SELECT * FROM crm_automations WHERE organization_id = ? AND enabled = 1 ORDER BY id ASC",[a]);return b}async function aw(a,b,c,d){await t();let e=q();await e.query("INSERT INTO crm_automation_runs (organization_id, automation_id, created_count, summary) VALUES (?, ?, ?, ?)",[a,b,c,d.slice(0,255)]),await e.query("UPDATE crm_automations SET last_run_at = CURRENT_TIMESTAMP, created_count = created_count + ? WHERE id = ? AND organization_id = ?",[c,b,a])}async function ax(a,b){await t();let[c]=await q().query("SELECT 1 FROM crm_tasks WHERE organization_id = ? AND title = ? AND done = 0 LIMIT 1",[a,b]);return c.length>0}async function ay(){await t();let[a]=await q().query("SELECT DISTINCT organization_id FROM crm_automations WHERE enabled = 1 ORDER BY organization_id");return a.map(a=>Number(a.organization_id))}function az(){return p.__crmAuthSchema||(p.__crmAuthSchema=(async()=>{let a=q();await a.query(`
      ...
      `)})().catch(a=>{throw p.__crmAuthSchema=void 0,a})),p.__crmAuthSchema}async function aA(){await az();let[a]=await q().query("SELECT COUNT(*) AS n FROM crm_users");return Number(a[0]?.n??0)}async function aB(a){await az();let[b]=await q().query("SELECT * FROM crm_organizations WHERE id = ? LIMIT 1",[a]);return b[0]??null}async function aC(a,b,c){await az(),await q().query("UPDATE crm_organizations SET name = ?, slug = ? WHERE id = ?",[b.slice(0,190),c.slice(0,120),a])}async function aD(a){await az();let[b]=await q().query("SELECT COUNT(*) AS n FROM crm_users WHERE organization_id = ?",[a]);return Number(b[0]?.n??0)}async function aE(a,b){await az(),await q().query("UPDATE crm_organizations SET plan = ? WHERE id = ?",[b.slice(0,24),a])}async function aF(a,b){await az(),await q().query("UPDATE crm_organizations SET billing_email = ?, billing_name = ?, billing_address = ?, tax_id = ? WHERE id = ?",[b.billingEmail.slice(0,190),b.billingName.slice(0,190),b.billingAddress.slice(0,500),b.taxId.slice(0,40),a])}async function aG(a){await t(),await az();let b=q(),c=c=>b.query(c,[a]).then(([a])=>Number(a[0]?.n??0)),[d,e,f,g]=await Promise.all([c("SELECT COUNT(*) AS n FROM crm_users WHERE organization_id = ?"),c("SELECT COUNT(*) AS n FROM crm_companies WHERE organization_id = ?"),c("SELECT COUNT(*) AS n FROM crm_contacts WHERE organization_id = ?"),c("SELECT COUNT(*) AS n FROM crm_deals WHERE organization_id = ?")]);return{users:d,companies:e,contacts:f,deals:g}}async function aH(a){await az();let[b]=await q().query("SELECT * FROM crm_api_keys WHERE organization_id = ? ORDER BY id DESC",[a]);return b}async function aI(a,b){await az();let[c]=await q().query("INSERT INTO crm_api_keys (organization_id, name, key_hash, last4, created_by_email) VALUES (?, ?, ?, ?, ?)",[a,b.name.slice(0,120),b.keyHash.slice(0,64),b.last4.slice(0,8),b.createdByEmail.slice(0,190)]);return c.insertId}async function aJ(a,b,c){await az(),await q().query("UPDATE crm_api_keys SET enabled = ? WHERE id = ? AND organization_id = ?",[+!!c,b,a])}async function aK(a,b){await az(),await q().query("DELETE FROM crm_api_keys WHERE id = ? AND organization_id = ?",[b,a])}async function aL(a){await az();let[b]=await q().query("SELECT id, organization_id FROM crm_api_keys WHERE key_hash = ? AND enabled = 1 LIMIT 1",[a]),c=b[0];return c?{id:c.id,organizationId:c.organization_id}:null}async function aM(a){await az(),await q().query("UPDATE crm_api_keys SET last_used_at = CURRENT_TIMESTAMP, request_count = request_count + 1 WHERE id = ?",[a]).catch(()=>{})}async function aN(a){await az();let[b]=await q().query("SELECT * FROM crm_users WHERE email = ? AND status = 'active' LIMIT 1",[a.toLowerCase()]);return b[0]??null}async function aO(a){await az();let[b]=await q().query("SELECT * FROM crm_users WHERE id = ? LIMIT 1",[a]);return b[0]??null}async function aP(a){await az(),await q().query("UPDATE crm_users SET last_login_at = CURRENT_TIMESTAMP WHERE id = ?",[a])}async function aQ(a){await az(),await q().query("INSERT INTO crm_sessions (user_id, organization_id, token_hash, expires_at) VALUES (?, ?, ?, ?)",[a.userId,a.organizationId,a.tokenHash,a.expiresAt])}async function aR(a){await az();let[b]=await q().query("SELECT * FROM crm_sessions WHERE token_hash = ? AND expires_at > CURRENT_TIMESTAMP LIMIT 1",[a]);return b[0]??null}async function aS(a){await az(),await q().query("INSERT INTO crm_audit_logs (organization_id, user_id, actor_email, action, entity, entity_id, summary) VALUES (?, ?, ?, ?, ?, ?, ?)",[a.organizationId,a.userId,a.actorEmail.slice(0,190),a.action.slice(0,40),a.entity.slice(0,40),a.entityId??null,(a.summary??"").slice(0,255)])}},50505:(a,b,c)=>{"use strict";c.d(b,{DY:()=>n,Hn:()=>l,Qq:()=>h,WI:()=>m,an:()=>d,dw:()=>k,ub:()=>o,w7:()=>f,yi:()=>j,zL:()=>e});let d=[{id:"new",label:"New lead",probability:10,open:!0},{id:"qualified",label:"Qualified",probability:20,open:!0},{id:"contacted",label:"Contacted",probability:30,open:!0},{id:"discovery",label:"Discovery",probability:45,open:!0},{id:"meeting",label:"Meeting",probability:60,open:!0},{id:"quote",label:"Quote sent",probability:75,open:!0},{id:"negotiation",label:"Negotiation",probability:85,open:!0},{id:"won",label:"Won",probability:100,open:!1},{id:"lost",label:"Lost",probability:0,open:!1}],e=d.filter(a=>a.open),f=d.map(a=>a.id),g=["price","competitor","no_budget","timing","no_response","wrong_fit","other"];function h(a){return g.includes(a)}let i=new Map(d.map(a=>[a.id,a]));function j(a){return i.has(a)}function k(a){return i.get(a)??d[0]}function l(a){return i.get(a)?.label??a}function m(a){let b=Object.fromEntries(f.map(a=>[a,{count:0,value:0}])),c=0,d=0,e=0,g=0,h=0,j=0;for(let f of a){let a=i.get(f.stage)?f.stage:"new";b[a].count+=1,b[a].value+=f.value,k(a).open?(c+=f.value,d+=function(a){let b=k(a.stage);if(!b.open)return"won"===b.id?a.value:0;let c=null!=a.probability?a.probability:b.probability;return Math.round(a.value*Math.max(0,Math.min(100,c))/100)}(f),g+=1):"won"===a?(e+=f.value,h+=1):j+=1}let l=h+j,m=l?Math.round(h/l*100):0;return{open:c,weighted:d,won:e,openCount:g,wonCount:h,lostCount:j,winRate:m,byStage:b}}function n(a){let b=30;a.hasWebsite&&(b+=15),a.industryMatch&&(b+=25);let c=a.employees??0;return c>=200?b+=20:c>=50?b+=15:c>=10&&(b+=8),(a.annualValue??0)>=2e4&&(b+=10),Math.max(0,Math.min(100,b))}function o(a){let b=[{label:"Base",points:30}];a.hasWebsite&&b.push({label:"Has a website",points:15}),a.industryMatch&&b.push({label:"Industry fit",points:25});let c=a.employees??0;return c>=200?b.push({label:"200+ employees",points:20}):c>=50?b.push({label:"50+ employees",points:15}):c>=10&&b.push({label:"10+ employees",points:8}),(a.annualValue??0)>=2e4&&b.push({label:"Annual value ≥ €20k",points:10}),{total:Math.max(0,Math.min(100,b.reduce((a,b)=>a+b.points,0))),factors:b}}},85750:(a,b,c)=>{"use strict";function d(a){return`Q-${String(a).padStart(4,"0")}`}c.d(b,{yx:()=>d})}};